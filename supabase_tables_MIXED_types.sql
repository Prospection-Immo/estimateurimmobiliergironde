-- SCRIPT AVEC TYPES MIXTES BASÉS SUR VOS ERREURS
-- guides.id = UUID, leads.id = VARCHAR
...
-- Guide Downloads (guides.id = UUID d'après l'erreur)
...
-- Guide Analytics (guides.id = UUID)
...
-- Guide Email Sequences (guides.id = UUID)
...
-- Lead Scoring (leads.id = VARCHAR d'après l'erreur)
...
-- Lead Score History (leads.id = VARCHAR)
...
SELECT 'TABLES CRÉÉES AVEC TYPES MIXTES CORRECTS!' as resultat;